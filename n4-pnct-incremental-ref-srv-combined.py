...
refBizScopedCon_ds = glueContext.create_dynamic_frame.from_catalog(database = "staging_incremental", table_name = "ref_bizunit_scoped", transformation_ctx = "refBizScopedCon_ds")
refBizScopedCon_regDF = refBizScopedCon_ds.toDF()
refBizScopedCon_regDF = refBizScopedCon_regDF.withColumn("sourcesystem", lit("PNCT")).withColumn("audtdateadded", lit(current_timestamp))
refBizScopedCon_dynDF = DynamicFrame.fromDF(refBizScopedCon_regDF, glueContext, "nested")

## ref_carrier_itinerary connection
refCarItinCon_ds = glueContext.create_dynamic_frame.from_catalog(database = "staging_incremental", table_name = "ref_carrier_itinerary", transformation_ctx = "refCarItinCon_ds")
refCarItinCon_regDF = refCarItinCon_ds.toDF()
refCarItinCon_regDF = refCarItinCon_regDF.withColumn("sourcesystem", lit("PNCT")).withColumn("audtdateadded", lit(current_timestamp))
refCarItinCon_dynDF = DynamicFrame.fromDF(refCarItinCon_regDF, glueContext, "nested")

## ref_carrier_service connection
refCarServCon_ds = glueContext.create_dynamic_frame.from_catalog(database = "staging_incremental", table_name = "ref_carrier_service", transformation_ctx = "refCarServCon_ds")
refCarServCon_regDF = refCarServCon_ds.toDF()
refCarServCon_regDF = refCarServCon_regDF.withColumn("sourcesystem", lit("PNCT")).withColumn("audtdateadded", lit(current_timestamp))
refCarServCon_dynDF = DynamicFrame.fromDF(refCarServCon_regDF, glueContext, "nested")

## ref_country connection
refCountryCon_ds = glueContext.create_dynamic_frame.from_catalog(database = "staging_incremental", table_name = "ref_country", transformation_ctx = "refCountryCon_ds")
refCountryCon_regDF = refCountryCon_ds.toDF()
refCountryCon_regDF = refCountryCon_regDF.withColumn("sourcesystem", lit("PNCT")).withColumn("audtdateadded", lit(current_timestamp))
refCountryCon_dynDF = DynamicFrame.fromDF(refCountryCon_regDF, glueContext, "nested")

## ref_equipment connection
refEquip_ds = glueContext.create_dynamic_frame.from_catalog(database = "staging_incremental", table_name = "ref_equipment", transformation_ctx = "refEquip_ds")
refEquip_regDF = refEquip_ds.toDF()
refEquip_regDF = refEquip_regDF.withColumn("sourcesystem", lit("PNCT")).withColumn("audtdateadded", lit(current_timestamp))
refEquip_dynDF = DynamicFrame.fromDF(refEquip_regDF, glueContext, "nested")

## ref_equip_type connection
refEquipType_ds = glueContext.create_dynamic_frame.from_catalog(database = "staging_incremental", table_name = "ref_equip_type", transformation_ctx = "refEquipType_ds")
refEquipType_regDF = refEquipType_ds.toDF()
refEquipType_regDF = refEquipType_regDF.withColumn("sourcesystem", lit("PNCT")).withColumn("audtdateadded", lit(current_timestamp))
refEquipType_dynDF = DynamicFrame.fromDF(refEquipType_regDF, glueContext, "nested")

## ref_line_operator connection
refLineOper_ds = glueContext.create_dynamic_frame.from_catalog(database = "staging_incremental", table_name = "ref_line_operator", transformation_ctx = "refLineOper_ds")
refLineOper_regDF = refLineOper_ds.toDF()
refLineOper_regDF = refLineOper_regDF.withColumn("sourcesystem", lit("PNCT")).withColumn("audtdateadded", lit(current_timestamp))
refLineOper_dynDF = DynamicFrame.fromDF(refLineOper_regDF, glueContext, "nested")

## ref_routing_point connection
refRoutingPCon_ds = glueContext.create_dynamic_frame.from_catalog(database = "staging_incremental", table_name = "ref_routing_point", transformation_ctx = "refRoutingPCon_ds")
refRoutingPCon_regDF = refRoutingPCon_ds.toDF()
refRoutingPCon_regDF = refRoutingPCon_regDF.withColumn("sourcesystem", lit("PNCT")).withColumn("audtdateadded", lit(current_timestamp))
refRoutingPCon_dynDF = DynamicFrame.fromDF(refRoutingPCon_regDF, glueContext, "nested")

## ref_unloc_code connection
refUC_ds = glueContext.create_dynamic_frame.from_catalog(database = "staging_incremental", table_name = "ref_unloc_code", transformation_ctx = "refUC_ds")
refUC_regDF = refUC_ds.toDF()
refUC_regDF = refUC_regDF.withColumn("sourcesystem", lit("PNCT")).withColumn("audtdateadded", lit(current_timestamp))
refUC_dynDF = DynamicFrame.fromDF(refUC_regDF, glueContext, "nested")

## srv_event connection
srvEV_ds = glueContext.create_dynamic_frame.from_catalog(database = "staging_incremental", table_name = "srv_event", transformation_ctx = "srvEV_ds")
srvEV_regDF = srvEV_ds.toDF()
srvEV_regDF = srvEV_regDF.withColumn("sourcesystem", lit("PNCT")).withColumn("audtdateadded", lit(current_timestamp))
srvEV_dynDF = DynamicFrame.fromDF(srvEV_regDF, glueContext, "nested")

## srv_event_types connection
srvEVType_ds = glueContext.create_dynamic_frame.from_catalog(database = "staging_incremental", table_name = "srv_event_types", transformation_ctx = "srvEVType_ds")
srvEVType_regDF = srvEVType_ds.toDF()
srvEVType_regDF = srvEVType_regDF.withColumn("sourcesystem", lit("PNCT")).withColumn("audtdateadded", lit(current_timestamp))
...
glue.start_job_run(JobName = 'n4-pnct-ref-srv-parquet')	
