...
# latch connection
latch_ds = glueContext.create_dynamic_frame.from_catalog(database="staging_incremental", table_name="latch",transformation_ctx="latch_ds")
latch_regDF = latch_ds.toDF()
latch_regDF = latch_regDF.withColumn("sourcesystem", lit("PNCT")).withColumn("audtdateadded", lit(current_timestamp))
latch_dynDF = DynamicFrame.fromDF(latch_regDF, glueContext, "nested")

# roverdetails connection
roverdetails_ds = glueContext.create_dynamic_frame.from_catalog(database="staging_incremental",table_name="roverdetails",transformation_ctx="roverdetails_ds")
roverdetails_regDF = roverdetails_ds.toDF()
roverdetails_regDF = roverdetails_regDF.withColumn("sourcesystem", lit("PNCT")).withColumn("audtdateadded",lit(current_timestamp))
roverdetails_dynDF = DynamicFrame.fromDF(roverdetails_regDF, glueContext, "nested")

# position connection
position_ds = glueContext.create_dynamic_frame.from_catalog(database ="staging_incremental", table_name ="position",transformation_ctx = "position_ds")
position_regDF = position_ds.toDF()
position_regDF = position_regDF.withColumn("sourcesystem", lit("PNCT")).withColumn("audtdateadded", lit(current_timestamp))
position_dynDF = DynamicFrame.fromDF(position_regDF, glueContext, "nested")

# mapdata connection
mapdata_ds = glueContext.create_dynamic_frame.from_catalog(database = "staging_incremental", table_name = "mapdata",transformation_ctx = "mapdata_ds")
mapdata_regDF = mapdata_ds.toDF()
mapdata_regDF = mapdata_regDF.withColumn("sourcesystem",lit("PNCT")).withColumn("audtdateadded",lit(current_timestamp))
mapdata_dynDF = DynamicFrame.fromDF(mapdata_regDF,glueContext,"nested")

# latchtype connection
latchtype_ds = glueContext.create_dynamic_frame.from_catalog(database = "staging_incremental", table_name = "latchtype",transformation_ctx = "latchtype_ds")
latchtype_regDF = latchtype_ds.toDF()
latchtype_regDF = latchtype_regDF.withColumn("sourcesystem",lit("PNCT")).withColumn("audtdateadded",lit(current_timestamp))
latchtype_dynDF = DynamicFrame.fromDF(latchtype_regDF,glueContext,"nested")

# utctimezoneoffsets connection
offset_ds = glueContext.create_dynamic_frame.from_catalog(database = "staging_incremental", table_name = "utctimezoneoffsets",transformation_ctx = "offset_ds")
offset_regDF = offset_ds.toDF()
offset_regDF = offset_regDF.withColumn("sourcesystem",lit("PNCT")).withColumn( "audtdateadded",lit(current_timestamp))
offset_dynDF = DynamicFrame.fromDF(offset_regDF,glueContext,"nested")

# vehicletype connection
vehicletype_ds = glueContext.create_dynamic_frame.from_catalog(database = "staging_incremental", table_name = "vehicletype",transformation_ctx = "vehicletype_ds")
vehicletype_regDF = vehicletype_ds.toDF()
vehicletype_regDF = vehicletype_regDF.withColumn("sourcesystem",lit("PNCT")).withColumn("audtdateadded",lit(current_timestamp))
vehicletype_dynDF = DynamicFrame.fromDF(vehicletype_regDF,glueContext,"nested")

# flagdescription connection
flagdesc_ds = glueContext.create_dynamic_frame.from_catalog(database = "staging_incremental", table_name = "flagdescription",transformation_ctx = "flagdesc_ds")
flagdesc_regDF = flagdesc_ds.toDF()
flagdesc_regDF = flagdesc_regDF.withColumn("sourcesystem",lit("PNCT")).withColumn("audtdateadded",lit(current_timestamp))
flagdesc_dynDF = DynamicFrame.fromDF(flagdesc_regDF,glueContext,"nested")

# maptype connection
maptype_ds = glueContext.create_dynamic_frame.from_catalog(database = "staging_incremental", table_name = "maptype", transformation_ctx = "maptype_ds")
maptype_regDF = maptype_ds.toDF()
maptype_regDF = maptype_regDF.withColumn("sourcesystem",lit("PNCT")).withColumn("audtdateadded",lit(current_timestamp))
maptype_dynDF = DynamicFrame.fromDF(maptype_regDF,glueContext,"nested")

## vehicledetails connection
vdetails_DS = glueContext.create_dynamic_frame.from_catalog(database = "staging_incremental", table_name = "vehicledetails", transformation_ctx = "vdetails_DS")
vdetails_regDF = vdetails_DS.toDF()
vdetails_regDF = vdetails_regDF.withColumn("sourcesystem",lit("PNCT")).withColumn("audtdateadded",lit(current_timestamp))
...
glue.start_job_run(JobName = 'mistar-pnct-dbo-parquet')	
