...
## impact connection
impact_ds = glueContext.create_dynamic_frame.from_catalog(database="staging_initial", table_name="impact",transformation_ctx="impact_ds")
impact_regDF = impact_ds.toDF()
impact_regDF = impact_regDF.withColumn("sourcesystem",lit("PNCT")).withColumn("dboperationtype",lit("L")).withColumn("audtdateadded",lit(current_timestamp))
impact_distDF =  impact_regDF.distinct()
impact_dynDF = DynamicFrame.fromDF(impact_distDF, glueContext, "nested")

## fuel connection
fuel_ds = glueContext.create_dynamic_frame.from_catalog(database="staging_initial", table_name="fuel",transformation_ctx="fuel_ds")
fuel_regDF = fuel_ds.toDF()
fuel_regDF = fuel_regDF.withColumn("sourcesystem",lit("PNCT")).withColumn("dboperationtype",lit("L")).withColumn("audtdateadded",lit(current_timestamp))
fuel_distDF =  fuel_regDF.distinct()
fuel_dynDF = DynamicFrame.fromDF(fuel_distDF, glueContext, "nested")

## tire connection
tire_ds = glueContext.create_dynamic_frame.from_catalog(database="staging_initial", table_name="tire",transformation_ctx="tire_ds")
tire_regDF = tire_ds.toDF()
tire_regDF = tire_regDF.withColumn("sourcesystem",lit("PNCT")).withColumn("dboperationtype",lit("L")).withColumn("audtdateadded",lit(current_timestamp))
tire_distDF =  tire_regDF.distinct()
tire_dynDF = DynamicFrame.fromDF(tire_distDF, glueContext, "nested")

## tiredetail connection
tiredet_ds = glueContext.create_dynamic_frame.from_catalog(database="staging_initial", table_name="tiredetail",transformation_ctx="tiredet_ds")
tiredet_regDF = tiredet_ds.toDF()
tiredet_regDF = tiredet_regDF.withColumn("sourcesystem", lit("PNCT")).withColumn("dboperationtype",lit("L")).withColumn("audtdateadded", lit(current_timestamp))
tiredet_dynDF = DynamicFrame.fromDF(tiredet_regDF, glueContext, "nested")

## tiresourcetype connection
tirest_ds = glueContext.create_dynamic_frame.from_catalog(database="staging_initial", table_name="tiresourcetype",transformation_ctx="tirest_ds")
tirest_regDF = tirest_ds.toDF()
tirest_regDF = tirest_regDF.withColumn("sourcesystem", lit("PNCT")).withColumn("dboperationtype",lit("L")).withColumn("audtdateadded", lit(current_timestamp))
tirest_dynDF = DynamicFrame.fromDF(tirest_regDF, glueContext, "nested")

## tiretype connection
tiretype_ds = glueContext.create_dynamic_frame.from_catalog(database="staging_initial", table_name="tiretype",transformation_ctx="tiretype_ds")
tiretype_regDF = tiretype_ds.toDF()
tiretype_regDF = tiretype_regDF.withColumn("sourcesystem", lit("PNCT")).withColumn("dboperationtype",lit("L")).withColumn("audtdateadded", lit(current_timestamp))
...
## impact datasink
impact_datasinkcombined = glueContext.write_dynamic_frame.from_options(frame=impact_mappingcombined, connection_type="s3",
                                                         connection_options={
                                                             "path": "s3://pa-dms-staging/mistar/pnct/combined/impact"},
                                                         format="parquet", transformation_ctx="impact_datasinkcombined")

## fuel datasink
fuel_datasinkcombined = glueContext.write_dynamic_frame.from_options(frame=fuel_mappingcombined, connection_type="s3",
                                                         connection_options={
                                                             "path": "s3://pa-dms-staging/mistar/pnct/combined/fuel"},
                                                         format="parquet", transformation_ctx="fuel_datasinkcombined")
## tire datasink
tire_datasinkcombined = glueContext.write_dynamic_frame.from_options(frame=tire_mappingcombined, connection_type="s3",
                                                         connection_options={
                                                             "path": "s3://pa-dms-staging/mistar/pnct/combined/tire"},
                                                         format="parquet", transformation_ctx="tire_datasinkcombined")
