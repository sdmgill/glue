...
dep_DS = glueContext.create_dynamic_frame.from_catalog(database = "staging_incremental", table_name = "department", transformation_ctx = "dep_DS")
dep_regDF = dep_DS.toDF()
dep_regDF.withColumn("sourcesystem",lit("MAINPAC")).withColumn("audtdateadded",lit(current_timestamp))
dep_dynDF = DynamicFrame.fromDF(dep_regDF,glueContext,"nested")

## location connection
loc_DS = glueContext.create_dynamic_frame.from_catalog(database = "staging_incremental", table_name = "location", transformation_ctx = "loc_DS")
loc_regDF = loc_DS.toDF()
loc_regDF.withColumn("sourcesystem",lit("MAINPAC")).withColumn("audtdateadded",lit(current_timestamp))
loc_dynDF = DynamicFrame.fromDF(loc_regDF,glueContext,"nested")

## lookuptables Connection
lupt_DS = glueContext.create_dynamic_frame.from_catalog(database = "staging_incremental", table_name = "lookuptable", transformation_ctx = "lupt_DS")
lupt_regDF = lupt_DS.toDF()
lupt_regDF.withColumn("sourcesystem",lit("MAINPAC")).withColumn("audtdateadded",lit(current_timestamp))
lupt_dynDF = DynamicFrame.fromDF(lupt_regDF,glueContext,"nested")

## operationalview connection
opview_DS = glueContext.create_dynamic_frame.from_catalog(database = "staging_incremental", table_name = "operationalview", transformation_ctx = "opview_DS")
opview_regDF = opview_DS.toDF()
opview_regDF.withColumn("sourcesystem",lit("MAINPAC")).withColumn("audtdateadded",lit(current_timestamp))
opview_dynDF = DynamicFrame.fromDF(opview_regDF,glueContext,"nested")

## worktype connection
workt_DS = glueContext.create_dynamic_frame.from_catalog(database = "staging_incremental", table_name = "worktype", transformation_ctx = "workt_DS")
workt_regDF = workt_DS.toDF()
workt_regDF.withColumn("sourcesystem",lit("MAINPAC")).withColumn("audtdateadded",lit(current_timestamp))
workt_dynDF = DynamicFrame.fromDF(workt_regDF,glueContext,"nested")

## tableattribute connection
tabatt_DS = glueContext.create_dynamic_frame.from_catalog(database = "staging_incremental", table_name = "tableattribute", transformation_ctx = "tabatt_DS")
tabatt_regDF = tabatt_DS.toDF()
tabatt_regDF.withColumn("sourcesystem",lit("MAINPAC")).withColumn("audtdateadded",lit(current_timestamp))
...
glue.start_job_run(JobName = 'mainpac-dbo-ref-tables1-parquet')
