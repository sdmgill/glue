...
argoCV_ds = glueContext.create_dynamic_frame.from_catalog(database = "staging_incremental", table_name = "argo_carrier_visit", transformation_ctx = "argoCV_ds")
argoCV_regDF = argoCV_ds.toDF()
argoCV_regDF = argoCV_regDF.withColumn("sourcesystem",lit("PNCT")).withColumn("audtdateadded",lit(current_timestamp))
argoCV_dynDF = DynamicFrame.fromDF(argoCV_regDF,glueContext,"nested")

## argo_chargeable_unit_events connection
argoCUE_ds = glueContext.create_dynamic_frame.from_catalog(database = "staging_incremental", table_name = "argo_chargeable_unit_events", transformation_ctx = "argoCUE_ds")
argoCUE_regDF = argoCUE_ds.toDF()
argoCUE_regDF = argoCUE_regDF.withColumn("sourcesystem",lit("PNCT")).withColumn("audtdateadded",lit(current_timestamp))
argoCUE_dynDF = DynamicFrame.fromDF(argoCUE_regDF,glueContext,"nested")

## argo_visit_details connection
argoVD_ds = glueContext.create_dynamic_frame.from_catalog(database = "staging_incremental", table_name = "argo_visit_details", transformation_ctx = "argoVD_ds")
argoVD_regDF = argoVD_ds.toDF()
argoVD_regDF = argoVD_regDF.withColumn("sourcesystem",lit("PNCT")).withColumn("audtdateadded",lit(current_timestamp))
argoVD_dynDF = DynamicFrame.fromDF(argoVD_regDF,glueContext,"nested")

## argo_yard connection
argoyard_ds = glueContext.create_dynamic_frame.from_catalog(database = "staging_incremental", table_name = "argo_yard", transformation_ctx = "argoyard_ds")
argoyard_regDF = argoyard_ds.toDF()
argoyard_regDF = argoyard_regDF.withColumn("sourcesystem",lit("PNCT")).withColumn("audtdateadded",lit(current_timestamp))
argoyard_dynDF = DynamicFrame.fromDF(argoyard_regDF,glueContext,"nested")

## crg_bills_of_lading connection
crgBOL_ds = glueContext.create_dynamic_frame.from_catalog(database = "staging_incremental", table_name = "crg_bills_of_lading", transformation_ctx = "crgBOL_ds")
crgBOL_regDF = crgBOL_ds.toDF()
crgBOL_regDF = crgBOL_regDF.withColumn("sourcesystem",lit("PNCT")).withColumn("audtdateadded",lit(current_timestamp))
crgBOL_dynDF = DynamicFrame.fromDF(crgBOL_regDF,glueContext,"nested")

## crg_bl_goods connection
crgBG_ds = glueContext.create_dynamic_frame.from_catalog(database = "staging_incremental", table_name = "crg_bl_goods", transformation_ctx = "crgBG_ds")
crgBG_regDF = crgBG_ds.toDF()
crgBG_regDF = crgBG_regDF.withColumn("sourcesystem",lit("PNCT")).withColumn("audtdateadded",lit(current_timestamp))
...
glue.start_job_run(JobName = 'n4-pnct-argo-crg-parquet')
