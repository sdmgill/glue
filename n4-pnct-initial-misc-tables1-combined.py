...
calev_DS = glueContext.create_dynamic_frame.from_catalog(database = "staging_initial", table_name = "argo_cal_event", transformation_ctx = "calev_DS")
calev_regDF = calev_DS.toDF()
calev_regDF = calev_regDF.withColumn("sourcesystem",lit("PNCT")).withColumn("dboperationtype",lit("L")).withColumn("audtdateadded",lit(current_timestamp))
calev_dynDF = DynamicFrame.fromDF(calev_regDF,glueContext,"nested")

## argo_facility connection
fac_DS = glueContext.create_dynamic_frame.from_catalog(database = "staging_initial", table_name = "argo_facility", transformation_ctx = "fac_DS")
fac_regDF = fac_DS.toDF()
fac_regDF = fac_regDF.withColumn("sourcesystem",lit("PNCT")).withColumn("dboperationtype",lit("L")).withColumn("audtdateadded",lit(current_timestamp))
fac_dynDF = DynamicFrame.fromDF(fac_regDF,glueContext,"nested")

## frm_mapping_predicates Connection
mappred_DS = glueContext.create_dynamic_frame.from_catalog(database = "staging_initial", table_name = "frm_mapping_predicates", transformation_ctx = "mappred_DS")
mappred_regDF = mappred_DS.toDF()
mappred_regDF = mappred_regDF.withColumn("sourcesystem",lit("PNCT")).withColumn("dboperationtype",lit("L")).withColumn("audtdateadded",lit(current_timestamp))
mappred_dynDF = DynamicFrame.fromDF(mappred_regDF,glueContext,"nested")

## frm_mapping_values connection
mapval_DS = glueContext.create_dynamic_frame.from_catalog(database = "staging_initial", table_name = "frm_mapping_values", transformation_ctx = "mapval_DS")
mapval_regDF = mapval_DS.toDF()
mapval_regDF = mapval_regDF.withColumn("sourcesystem",lit("PNCT")).withColumn("dboperationtype",lit("L")).withColumn("audtdateadded",lit(current_timestamp))
mapval_dynDF = DynamicFrame.fromDF(mapval_regDF,glueContext,"nested")

## inv_storage_rule connection
storrul_DS = glueContext.create_dynamic_frame.from_catalog(database = "staging_initial", table_name = "inv_storage_rule", transformation_ctx = "storrul_DS")
storrul_regDF = storrul_DS.toDF()
storrul_regDF = storrul_regDF.withColumn("sourcesystem",lit("PNCT")).withColumn("dboperationtype",lit("L")).withColumn("audtdateadded",lit(current_timestamp))
storrul_dynDF = DynamicFrame.fromDF(storrul_regDF,glueContext,"nested")

## road_document_messages connection
docmes_DS = glueContext.create_dynamic_frame.from_catalog(database = "staging_initial", table_name = "road_document_messages", transformation_ctx = "docmes_DS")
docmes_regDF = docmes_DS.toDF()
docmes_regDF = docmes_regDF.withColumn("sourcesystem",lit("PNCT")).withColumn("dboperationtype",lit("L")).withColumn("audtdateadded",lit(current_timestamp))
